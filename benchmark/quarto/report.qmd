...
The **DORA** acronym used throughout this report stands for **DevOps Research
and Assessment** — the Google research programme that defines the four key
software-delivery metrics (Deployment Frequency, Lead Time for Changes, Change
Failure Rate, MTTR). This is distinct from the EU financial regulation of the
same acronym (Digital Operational Resilience Act, 2022/2554).

The DORA metrics (DevOps Research and Assessment) are computed from the timestamps
stored in `gh_releases.published_at_ts` and from the `ci_runs` table.
Results land in the **Elite** tier as defined by Forsgren et al.,
*Accelerate State of DevOps Report 2023* (Google Cloud / DORA,
\href{https://dora.dev/research/2023/dora-report/}{dora.dev/research/2023}).
