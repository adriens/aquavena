...
# Curated, human-readable cluster labels (data-driven via top terms,
# but trimmed to short phrases that fit a legend).
cluster_labels <- c(
  "0" = "Site features & About",
  "1" = "Style & dark mode",
  "2" = "Benchmark & ML report",
...
- **v0.1.2–v0.1.4** (the very first releases) are 100% *WCAG fixes* —
  the foundational accessibility work: contrast corrections, pa11y
  audit setup, WCAG 2.1 AA compliance from the ground up. This is the
  *raison d'être* of the project, visible in its purest form.
- **v0.1.5** is the **first multi-cluster release**: WCAG work is
  still present, but *Style \& dark mode*, *Site features \& About*
  and *Benchmark \& ML report* clusters all appear simultaneously.
  This is the moment the project diversified — from one concern
  (make it WCAG-clean) to four (style polish, feature additions,
  benchmark scaffolding).
- **v1.7.0** stands out as the only mid-life release where the
  *Site features \& About* cluster occupies a large share — this
  matches the historical record (RSS, iCal, About-page content all
  landed around this tag).
- **v1.9.0 → v1.13.x** are dominated by the *Benchmark \& ML report*
  cluster — the long **report-engineering phase**: this is where the
  100-page PDF you are reading was built. Feature work shifted to a
  data-science/reporting register.
- **v1.12.1** is the only release of this phase where *WCAG fixes*
  re-emerge at parity with the report work — likely a regression catch
  that triggered contrast revalidation.
- **v1.14.0** closes the chart with a return to 100% *WCAG fixes* —
  the **Tailwind v4 migration** release: a platform overhaul whose
  primary risk was contrast regression, so most of the work was
  re-validating accessibility.

Note: the chart stops at `v1.14.0` because newer release tags
(`v1.15.0`–`v1.24.0`) — including the entire WebMCP \& agentic-pipeline
work documented in this report — were published *after* the
`gh_releases` snapshot used by this build. Re-running the pipeline will
extend the timeline and reveal the *Site features \& About* and
*Benchmark \& ML report* clusters dominating the rightmost releases —
the agentic pivot's signature.
...
- An input to **agentic pipelines**: the AI agents from Section
  *Universal Accessibility Pipeline* can pre-warm a context for the
  likely next commit type — e.g. after a *WCAG fixes* commit, the agent
  loads contrast-audit tools because $P(\text{next} = \text{WCAG fixes})$
  is high.
- A regression detector at the **process level**: if the empirical mix
  ...
  *Style \& dark mode* after a *Benchmark \& ML report* commit), it
...
# Cluster id → friendly label (consistent with the rest of the report)
markov_labels <- c(
  "0" = "Site features\n& About",
  "1" = "Style &\ndark mode",
  "2" = "Benchmark\n& ML report",
...
- **For the developer**: post-hoc retrospective — *"on day X around
  17h I was stuck for 90 min on the WCAG fixes cluster"*. A coachable
  signal.
