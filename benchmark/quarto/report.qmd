...
# Derive chronological tag order from commit timestamps (avoids hardcoded list → no NA)
tag_chron <- tag_rows |>
  dplyr::distinct(tag, commit_ts) |>
  dplyr::arrange(commit_ts) |>
  dplyr::pull(tag)
...
p <- ggplot() +
  ...
  geom_text(data = cum_df,
            ...
            colour = "#92400e", size = 2.4, fontface = "bold", vjust = -1) +
