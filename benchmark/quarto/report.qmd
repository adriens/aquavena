...
ggplot(grid, aes(x = date, y = hour)) +
  ...
  scale_x_date(breaks = unique(grid$date),
               labels = format(unique(grid$date), "%a\n%d %b"),
               expand = c(0.01, 0)) +
...
if (nrow(cdrift_commits) > 0 && nrow(cdrift_releases) >= 3) {
  ...
  # Build chronological factor for x-axis (only keep releases that have commits)
...
releases_key <- releases_key[order(releases_key$bin_idx), ]
# Alternating heights to avoid label overlap
...
mix_bar <- ai_lev |>
  dplyr::group_by(cluster_label, model_short) |>
  dplyr::summarise(
    tokens_per_commit_M = sum(tokens, na.rm = TRUE) / max(n_commits) / 1e6,
    .groups = "drop"
  )
