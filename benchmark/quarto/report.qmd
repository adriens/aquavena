  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \usepackage{hyperref}
        \usepackage{tikz}
        \usetikzlibrary{positioning,shapes.geometric,shapes.multipart,arrows.meta,fit,backgrounds}
...
## AI-Agent Accessibility Testing Pipeline

The next frontier goes beyond static audits: **AI agents simulating real
disability profiles** — blind, dyslexic, low-vision, colorblind — interact
with the site and report perceived issues, not just rule violations. The
diagram below illustrates this envisioned pipeline, directly inspired by the
WebMCP infrastructure already in place.
...
\begin{tikzpicture}[
  ...
  agent/.style={draw=teal!70!black, fill=teal!8, rounded corners=4pt,
                minimum width=3.0cm, minimum height=0.7cm, align=center},
  box/.style={draw=gray!60, fill=gray!6, rounded corners=4pt,
              minimum width=3.2cm, align=center, inner sep=6pt},
  db/.style={draw=amber!80!black, fill=amber!10, cylinder, shape border rotate=90,
             aspect=0.25, minimum width=2.2cm, minimum height=0.8cm, align=center},
  arr/.style={-{Stealth[length=5pt]}, gray!70, line width=0.8pt},
  thick arr/.style={-{Stealth[length=6pt]}, teal!60!black, line width=1.2pt},
]

% ── AI agents (left column) ──────────────────────────────────────────────────
\node[agent] (blind)    at (0, 4.2) {Agent aveugle};
\node[agent] (dyslexic) at (0, 3.2) {Agent dyslexique};
\node[agent] (lowvis)   at (0, 2.2) {Agent malvoyant};
\node[agent] (color)    at (0, 1.2) {Agent daltonien};
\node[agent,draw=violet!60,fill=violet!6] (custom) at (0, 0.2) {Agent custom X};

% fine-tuning label
\node[align=center, font=\footnotesize\itshape, text=gray] (ft) at (0, 5.1)
  {Fine-tuning par profil};
\draw[arr] (ft.south) -- ++(0,-0.35);

% ── Multimodal scene (centre) ────────────────────────────────────────────────
\node[box, minimum height=2.8cm] (scene) at (4.8, 2.2) {
  \textbf{Interface multimodale}\\[4pt]
  \faVolumeUp\ voix\\
  \faImage\ image\\
  \faFont\ texte\\[4pt]
  \faPlug\ \texttt{WebMCP}
...
% arrows agents → scene
\foreach \ag in {blind,dyslexic,lowvis,color,custom}
  \draw[arr] (\ag.east) -- (scene.west |- \ag);

% ── Scout + PA11y (right) ────────────────────────────────────────────────────
\node[box] (scout) at (9.2, 3.4) {\textbf{Scout}\\pa11y-ci\\Lighthouse};
\node[db]  (db)    at (9.2, 1.2) {\textbf{DuckDB}\\score\_history};

\draw[thick arr] (scene.east) -- node[above,font=\footnotesize]{audit} (scout.west);
\draw[thick arr] (scout.south) -- node[right,font=\footnotesize]{persist} (db.north);

% ── Human test (bottom right) ────────────────────────────────────────────────
\node[box,draw=rose!60!black,fill=rose!6] (human) at (9.2,-0.8)
  {\faUser\ Test humain\\verbatim};
\draw[arr] (db.south) -- (human.north);

% ── Feedback loop ────────────────────────────────────────────────────────────
\draw[thick arr, bend right=35]
  (human.west) to node[below,font=\footnotesize\itshape]{feedback \& verbatim} (ft.east |- blind.south);

% ── Top banner ───────────────────────────────────────────────────────────────
\node[draw=teal!80!black, fill=teal!15, rounded corners=4pt,
      minimum width=11.5cm, minimum height=0.7cm, align=center,
      font=\bfseries\small] at (4.8, 5.6)
  {\faRobot\ AI-Actual Test — profils utilisateurs simulés};

\end{tikzpicture}
\caption{Pipeline envisagé : agents IA spécialisés par profil de handicap,
  interface multimodale WebMCP, audit automatisé et boucle de feedback humain.}
\end{figure}
```

The key insight is that **WebMCP is the natural interface** between the
site and these agents: instead of navigating a DOM, an agent calls
`get_week_menu` or `search_dish` and receives structured data — exactly as
a sighted user would read the page, but in machine-consumable form.

Each agent version is tagged with a dedicated semantic (e.g.
`agent-blind-v1.2`) so that score regressions can be attributed to a
specific simulation profile and retested in isolation.
