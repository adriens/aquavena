...
This heatmap plots **every commit by its local hour of day** (rows, midnight to
23h, Pacific/Noumea) across the project's calendar (columns). Darker tiles mean
more commits landed in that hour; the dotted lines mark the rough **day**
(≈ 06–18h) and **night** bands.

```{r activity-heatmap, fig.height=2.8, fig.width=6.5, fig.align="center"}
...
ggplot(grid, aes(x = date, y = hour)) +
  geom_tile(aes(fill = commits), colour = "white", linewidth = 0.3) +
  scale_fill_gradientn(
    colours = c("#f1f5f9", "#99f6e4", "#0f766e", "#134e4a"),
    values  = scales::rescale(c(0, 1, 4, max(grid$commits))),
    name    = "commits",
    guide   = guide_colourbar(barwidth = 6, barheight = 0.4,
                               title.position = "left", title.vjust = 0.8)
  ) +
  geom_hline(yintercept = 5.5,  linetype = "dotted", colour = "#94a3b8", linewidth = 0.4) +
  geom_hline(yintercept = 17.5, linetype = "dotted", colour = "#94a3b8", linewidth = 0.4) +
  annotate("text", x = min(grid$date), y = 11.5,
           label = "day", size = 2.2, colour = "#94a3b8", hjust = -0.1) +
  annotate("text", x = min(grid$date), y = 21,
           label = "night", size = 2.2, colour = "#94a3b8", hjust = -0.1) +
  scale_x_date(breaks = (function(d) {
                 # Show ~10 labels max, evenly spaced; always include first and last
                 step  <- max(1, ceiling(length(d) / 10))
                 picks <- unique(c(d[seq(1, length(d), by = step)], d[length(d)]))
                 picks
               })(unique(grid$date)),
               labels = function(x) format(x, "%a %d %b"),
               expand = c(0.01, 0)) +
  scale_y_reverse(breaks = c(0, 6, 12, 18, 23),
                  labels = c("00h","06h","12h","18h","23h"),
                  expand = c(0.01, 0)) +
  labs(title = sprintf("%d commits · %d active days · peak %d/h",
                       total_commits, active_days, peak$commits[1]),
       x = NULL, y = NULL) +
...
**What it reveals.** The cadence is unmistakably **bursty**: the commits land
on a handful of active days out of the full calendar span — long idle stretches
punctuated by short, intense sessions (the peak hour alone holds 20+ commits).
Activity clusters in two waves — the late-May *founding* sprint and the mid-June
*analysis* sprint — and spills well into the night band. This is the rhythm of
focused, hackathon-style delivery rather than a steady 9-to-5 cadence, and it is
the same two-wave structure the concept-emergence timeline recovers later from
the commit text alone.
...
# across every regeneration of the commit graph. Defined once, reused below.
cluster_label_map <- function() {
  ...
  # commit_clusters.terms may be stored as a list/array → coerce to a string
