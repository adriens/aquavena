...
gr_data <- tibble::tibble(
  ...
  author        = rep("adriens", 25),
  is_prerelease = rep(FALSE, 25),
  body_excerpt  = c(
...
All **146 commit messages** were embedded with the
...
**Why this matters at scale.** With 146 commits the graph is small, but the
...
if (nrow(cdrift_commits) > 0 && nrow(cdrift_releases) >= 3) {
  ...
  ggplot(counts, aes(x = tag, y = n, fill = label)) +
    ...
    labs(
      ...
      subtitle = "Stacked share of each commit cluster across the release timeline · 146 commits, 26 releases",
...
**What it is.** Sort all 146 commits in chronological order, then for every
...
if (n >= 20) {
  ...
  ggplot(trans_counts, aes(x = to_lbl, y = from_lbl, fill = prob)) +
    ...
    labs(
      ...
      subtitle = "Probability of moving from each cluster (row) to each next cluster (column) · 146 commits",
...
The 73.3\% number is easy to skim past. It is worth unpacking, because
...
| Majority-class predictor (always pick the largest cluster) | $\approx 7\%$ |
| GraphSAGE (this experiment) | $\boldsymbol{73\%}$ |

A model with no real signal would land between 7\% and 17\%. Landing
at 73\% rules out any explanation by chance: \textbf{the graph
...
4. \textbf{The honest limit.} On clusters with only 1–3 commits in the
   ...
   small-sample noise, not a finding. The +66.7-point lift on overall
...
\caption{Causal DAG learnt by the PC algorithm with Fisher's Z conditional
  independence test ($\alpha = 0.05$) on 146 commit-level observations of
...
\textbf{2. Opus usage drives both tokens and time-of-day.}
\emph{Opus \% → Tokens} is expected — Opus is 5× the price of Sonnet, so
sessions routed to it generate larger raw counts even at the same
"effort". The more interesting arrow is \emph{Opus \% → Hour of day}: PC
identifies a directed relationship between the choice of model and the
time the commit lands. Looking at the raw data, Opus-heavy commits cluster
in late-evening NC time — the moment when the developer reaches for the
premium model. **The model-routing decision precedes the time stamp.**

\textbf{3. \emph{Has scope} sits at the intersection of three forces.}
Three arrows converge on \emph{has\_scope}: \emph{fix → has\_scope},
\emph{weekend → has\_scope}, and (via the binary structure) the commit
type. This is the algorithm's way of saying: *the presence of a
conventional-commit scope is jointly determined by what kind of commit
it is and when it was written*. Weekend commits in particular score
higher on the scope discipline — possibly because they are smaller,
intentional, focused changes, where stating \texttt{fix(a11y):} comes
naturally.

\textbf{4. The graph-position triangle (\emph{PageRank, betweenness,
degree}) is internally consistent.} PC orients arrows from PageRank to
degree and from betweenness to degree, but leaves \emph{PageRank ↔
betweenness} undirected (dashed). This is structurally correct: in a
similarity graph, PageRank and betweenness measure different but
overlapping flavours of centrality and the data cannot distinguish which
one is "earlier". Treating both as **drivers of the degree** is
algebraically reasonable.
...
| Embedding-based effort prediction | ~500 commits | 146 |
...
## New perspectives unlocked by the 146-commit dataset

With 146 commits across 6 semantic clusters, 26 release tags, and a
...
: The join `commit_classification ↔ score_history` now has 26 labelled rows
  ...
  distribution across 146 commits is rich enough to train a lightweight
...
: `gh_releases → score_history → git_commits ← ci_runs ← pa11y_issues`
  is a heterogeneous knowledge graph. With 26 releases and 146 commits, a
