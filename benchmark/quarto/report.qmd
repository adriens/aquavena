...
The four metrics are mapped to four performance tiers. Aquavena lands in the
...
Aquavena actually lands (always **Elite**) is outlined and stamped with the
project's measured KPI.
...
ggplot(df, aes(x = tier, y = metric)) +
  ...
  annotate("text", x = 1, y = length(metrics) + 0.72, label = "▼ Aquavena",
  ...
  labs(title = "DORA scorecard — Elite on all four metrics",
       subtitle = "Colour = performance tier · outlined cell + bold value = Aquavena's measured KPI",
       x = NULL, y = NULL) +
  theme_minimal(base_size = 9) +
