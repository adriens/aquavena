...
```{r retro-gantt, fig.height=5.2, fig.width=7.0, fig.align="center"}
# BUILD = feat: commits + docs(benchmark): (the report is a deliverable)
# RUN   = everything else: fix:, style:, chore:, ci:, refactor:, other docs:
...
# Time-spent per commit: gap since previous commit across ALL commits, capped at 30 min.
GAP_CAP <- 30L * 60L
...
type_pal <- c(feat = "#0f766e", fix = "#dc2626", style = "#f59e0b",
              chore = "#9ca3af", docs = "#0891b2", ci = "#7c3aed",
              refactor = "#db2777")
swim_pal <- c(BUILD = "#0f766e", RUN = "#dc2626")

ggplot() +
  geom_col(data = intensity,
           aes(x = time_bin, y = active_min, fill = activity),
           width = BIN_SECS * 0.82, alpha = 0.55) +
  geom_vline(data = releases_br,
             aes(xintercept = published_at_ts),
             linetype = "dashed", colour = "#64748b",
             alpha = 0.50, linewidth = 0.28) +
  geom_rug(data = commits_br,
           aes(x = commit_ts, colour = commit_type),
           sides = "b", linewidth = 0.85,
           length = unit(0.07, "npc"), alpha = 0.85) +
  geom_text(data = releases_br,
            aes(x = published_at_ts, y = Inf, label = tag),
            angle = 60, hjust = 0, vjust = 0.5,
            size = 1.9, colour = "#475569", fontface = "bold",
            inherit.aes = FALSE) +
  scale_fill_manual(values = swim_pal, guide = "none") +
  scale_colour_manual(values = type_pal, name = NULL,
  ...
  labs(title = "BUILD vs RUN — hackathon intensity over time",
       subtitle = paste0("BUILD = feat: + docs(benchmark): · RUN = fix:/style:/chore:/ci: · ",
                         "bars = active minutes per 2 h window · rug = individual commits"),
       x = NULL, y = "active minutes") +
  theme_minimal(base_size = 9) +
  theme(panel.grid.minor   = element_blank(),
        panel.grid.major.x = element_blank(),
        strip.text  = element_text(face = "bold", size = 9),
        legend.position = "bottom",
        plot.margin = margin(30, 10, 6, 6),
