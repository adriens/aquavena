...
# ── ci_runs: record one row per pipeline execution ────────────────────────
ci_row <- tibble(
  run_id      = Sys.getenv("GITHUB_RUN_ID",   unset = paste0("local-", format(Sys.time(), "%Y%m%d%H%M%S"))),
  run_number  = as.integer(Sys.getenv("GITHUB_RUN_NUMBER", unset = "0")),
  git_sha     = Sys.getenv("GITHUB_SHA",      unset = system("git rev-parse --short HEAD 2>/dev/null", intern=TRUE)[1]),
  git_branch  = Sys.getenv("GITHUB_REF_NAME", unset = system("git rev-parse --abbrev-ref HEAD 2>/dev/null", intern=TRUE)[1]),
  triggered_by= if (nzchar(Sys.getenv("CI"))) "ci" else "local",
  audit_date  = AUDIT_DATE,
  lh_score    = lh_score,
  pa11y_errors= nrow(filter(pa11y_rows, type == "error"))
)
if (!dbExistsTable(con, "ci_runs")) {
  dbWriteTable(con, "ci_runs", ci_row)
} else {
  dbAppendTable(con, "ci_runs", ci_row)
}
```
