...
- An **abrupt cluster appearing late** → the project pivoted (new
  domain, new platform, new audience). On Aquavena, *Cluster 1
  (Features \& WebMCP)* appears only in the final third of the
  timeline: this is the visual signature of the agentic-interface
  pivot that this report itself documents.
...
- **v0.1.2–v0.1.4** (the very first releases) are 100% *SDK & meta* —
  pure scaffolding work to bring the codebase into existence.
- **v0.1.5** is the **first multi-cluster release**: SDK setup is still
  present, but *Style \& polish*, *Features \& WebMCP* and
  *Benchmark \& report* clusters all appear simultaneously. This is
  the moment the project diversified — from a single concern (set up
  the site) to multiple concerns (refine UX, publish features, document
  the work).
- **v1.7.0** stands out as the only mid-life release where the
  *Features \& WebMCP* cluster occupies a large share — this matches
  the historical record (early MCP-skill experiments were introduced
  around this tag).
- **v1.9.0 → v1.13.x** are dominated by the *UX fixes* cluster — the
  long **stabilisation phase**: dark mode, swipe navigation, header
  refinements, contrast corrections. Pure polish, no new shape.
- **v1.12.1** is the only release of this phase where SDK work re-emerges
  at parity with UX — likely a deps/tooling bump that touched
  cross-cutting code.
- **v1.14.0** closes the chart with a return to 100% *SDK \& meta* —
  the **Tailwind v4 migration** release, a pure platform overhaul.

Note: the chart stops at `v1.14.0` because newer release tags
(`v1.15.0`–`v1.23.0`) — including the entire WebMCP \& agentic-pipeline
...
extend the timeline and reveal the *Features \& WebMCP* and
*Benchmark \& report* clusters dominating the rightmost releases — the
agentic pivot's signature.
